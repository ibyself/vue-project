<template>
    <div class="star star-24" >
        <span class="star-item" :class="item" v-for="(item,index) in score" :key="index"></span>
    </div>
</template>
    ...
    export default {
        ...
        computed:{
            score(){
                ...
                //满星
                let allStar=Math.floor(rating)
                for (let index = 0; index < allStar; index++) {
                    result.push('on')
                }
                //半星
                (rating-allStar>0) && result.push('half')
